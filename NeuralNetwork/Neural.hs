type Dados = (Double, Double, Double, Double, Double, Double)
...
dsigma :: Double -> Double
dsigma x = sigma x * (1 - sigma x)

-- Função para calcular a saída da rede neural para uma entrada de dados
...
-- Função para calcular o gradiente do erro quadrático médio em relação aos pesos e ao viés da rede neural
gradiente :: [Double] -> [Dados] -> [Double]
gradiente ws dados =
  let n = fromIntegral (length dados)
      dedw1 = sum [ 2 * (predict ws d - y) * dsigma (predict ws d) * x1 | d@(x1, _, _, _, _, y) <- dados] / n
      dedw2 = sum [ 2 * (predict ws d - y) * dsigma (predict ws d) * x2 | d@(_, x2, _, _, _, y) <- dados] / n
      dedw3 = sum [ 2 * (predict ws d - y) * dsigma (predict ws d) * x3 | d@(_, _, x3, _, _, y) <- dados] / n
      dedw4 = sum [ 2 * (predict ws d - y) * dsigma (predict ws d) * x4 | d@(_, _, _, x4, _, y) <- dados] / n
      dedw5 = sum [ 2 * (predict ws d - y) * dsigma (predict ws d) * x5 | d@(_, _, _, _, x5, y) <- dados] / n
      dedb =  sum [ 2 * (predict ws d - y) * dsigma (predict ws d)      | d@(_, _, _, _, _, y)  <- dados] / n
...
-- Função para treinar a rede neural usando o método do gradiente descendente por um número fixo de épocas
update :: Int -> Double -> [Double] -> [Dados] -> [Double]
update 0 _ ws _ = ws
update passos lr ws dados =
  let novosPesos = atualizaPesos lr ws dados
   in update (passos - 1) lr novosPesos dados

-- Exemplo de uso das funções acima para treinar a rede neural com os dados fornecidos pelo usuário
...
main = do
  let passos = 0 --
      lr = 0.01
      pesosinicio = [3.2831774297364507,2.0683092898755584,13.187685985405743,-1.8089015482574466,1.0111544562453125,0.314267956279973] -- Valores iniciais dos pesos (w1, w2, w3, w4, w5, b)
  ...
  putStrLn "\nPrevisões:"
  let dadosTeste = (21.56, 22.39, 142.00, 1479.0, 0.11100, 955) -- onde fica 999 pode ser qualquer valor que o algoritmo vai prever
  let previsoes = predict pesosFinais dadosTeste
  print previsoes

--[ Testes ]
--11.760, 21.60, 74.72, 427.9, 0.08637, 1
--13.640, 16.34, 87.21, 571.8, 0.07685, 1
--11.940, 18.24, 75.71, 437.6, 0.08261, 1
--18.220, 18.70, 120.30, 1033.0, 0.11480, 0
--15.100, 22.02, 97.26, 712.8, 0.09056, 0
--11.520, 18.75, 73.34, 409.0, 0.09524, 1
--19.210, 18.57, 125.50, 1152.0, 0.10530, 0
--14.710, 21.59, 95.55, 656.9, 0.11370, 0
--13.050, 19.31, 82.61, 527.2, 0.08060, 1
--8.618, 11.79, 54.34, 224.5, 0.09752, 1
--10.170, 14.88, 64.55, 311.9, 0.11340, 1

-- [3.2831774297364507,2.0683092898755584,13.187685985405743,-1.8089015482574466,1.0111544562453125,0.314267956279973] -- Valores iniciais dos pesos (w1, w2, w3, w4, w5, b)